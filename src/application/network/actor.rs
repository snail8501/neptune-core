...
impl NetworkActor {
    ...
    pub fn new(
        ...
        if neuter_autonat {
            const TEN_THOUSAND_DAYS: Duration = Duration::from_secs(24 * 10_000 * 3600);
                    ...
                    for (multiaddr, sticky_peer) in &mut self.sticky_peers {
                        ...
                        match sticky_peer.clone() {
                            ...
                            StickyPeer::Dialing(since_timestamp) => {
                                ...
                                // try again.
                                if now.duration_since(since_timestamp).ok().is_some_and(|duration| duration > Duration::from_secs(2 * 60)) {
